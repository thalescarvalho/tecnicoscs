...
import MeusAgendamentos from "./pages/vendedor/MeusAgendamentos";
...
function NoRolePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-orange-100 flex flex-col items-center justify-center px-6 text-center space-y-4">
      ...
      <p className="text-muted-foreground">Sua conta ainda não tem um papel atribuído (Gestor, Técnico ou Vendedor).<br/>Peça ao administrador para configurar seu acesso.</p>
...
function RoleRoute({ children, allowedRoles }: { children: React.ReactNode; allowedRoles: string[] }) {
  ...
  if (!allowedRoles.includes(effectiveRole)) {
    const redirectTo = effectiveRole === 'gestor' ? '/dashboard' : effectiveRole === 'vendedor' ? '/vendedor/trabalhos' : '/meus-trabalhos';
...
function AuthRedirect() {
  ...
  if (!user) return <Navigate to="/login" replace />;
  if (!role) return <NoRolePage />;
  if (role === 'vendedor') return <Navigate to="/vendedor/trabalhos" replace />;
